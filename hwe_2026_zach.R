...
test_fun3 <- function(x,y) {
  ...
  return(data.table(x=x, y1=y1, y2=y2, y3=y3))
}
...
out2 <- test_fun3(x=c(0:100)/100)



### method 1: manually code the two y-values.
ggplot(data=out2) +
  geom_line(aes(x=x, y=pp), color="blue", name=qq) +
  geom_line(aes(x=x, y=pq), color="orange", name=pp)
  geom_line(aes (x=x, y=qq), color="green", name=pq)

### convert wide to long and use groupings in ggplot
out2_long <- melt(out2, id.vars="x", measure.vars=c("y1", "y2", "y3"))
